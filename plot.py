...
class CartoBase(object):
    ...
    def ovrlay_radar_data(self, rad, cbar=False, cbarh=True):
        data = rad.df[
...
def plot_TEC_TS():
    fig = plt.figure(figsize=(8, 3*3), dpi=150)
    axes = [fig.add_subplot(311+i) for i in range(3)]
    for ax in axes:
        ax.xaxis.set_major_formatter(DateFormatter(r"%H^{%M}"))
